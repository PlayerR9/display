...
// as it was an array of elements of type *ColoredUnit.
//
// Example:
//
//	[ a b c ]
...
func (t *Table) Iterator() common.Iterater[*ColoredUnit] {
	iter := common.NewDynamicIterator(
		common.NewSimpleIterator(t.table),
		func(row []*ColoredUnit) common.Iterater[*ColoredUnit] {
...
//
// It sets all cells in the table to the zero value of type *ColoredUnit.
func (t *Table) Cleanup() {
	for i := 0; i < t.height; i++ {
		t.table[i] = make([]*ColoredUnit, t.width)
	}
}

// NewTable creates a new table of type *ColoredUnit with the given width and height.
...
func NewTable(width, height int) *Table {
	...
	table := make([][]*ColoredUnit, height)
	for i := 0; i < height; i++ {
		table[i] = make([]*ColoredUnit, width)
	}
...
func (t *Table) WriteAt(x, y int, cell *ColoredUnit) {
...
// coordinates return the zero value of type *ColoredUnit.
//
// Parameters:
//   - x: The x-coordinate of the cell.
//   - y: The y-coordinate of the cell.
//
// Returns:
//   - *ColoredUnit: The cell at the given coordinates.
func (t *Table) GetAt(x, y int) *ColoredUnit {
	if x < 0 || x >= t.width || y < 0 || y >= t.height {
...
func (t *Table) WriteVerticalSequence(x, y *int, sequence []*ColoredUnit) {
	if x == nil || y == nil {
		return
	}
...
//   - y: The y-coordinate of the starting cell.
//   - sequence: The sequence of cells to write to the table.
func (t *Table) WriteHorizontalSequence(x, y *int, sequence []*ColoredUnit) {
	if x == nil || y == nil {
		return
	}
...
// GetFullTable returns the full table as a 2D slice of elements of type *ColoredUnit.
//
// Returns:
//   - [][]*ColoredUnit: The full table.
func (t *Table) GetFullTable() [][]*ColoredUnit {
	return t.table
}
